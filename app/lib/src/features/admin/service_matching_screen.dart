...
class _ServiceAdminAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    return AppBar(
      ...
      bottom: const TabBar(
        tabs: [
          Tab(text: 'مراجعة العروض', icon: Icon(Icons.verified_user_outlined)),
          Tab(text: 'طلبات الخدمات', icon: Icon(Icons.hub_outlined)),
        ],
      ),
    );
  }
...
class _PendingOffersTabState extends State<_PendingOffersTab> {
  ...
  Future<void> _decide(String id, bool approve) async {
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر تحديث العرض: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) return Center(child: Text('تعذر تحميل عروض المهارات: ${snapshot.error}'));
        final rows = snapshot.data ?? const [];
        if (rows.isEmpty) return const _AdminEmpty('لا توجد عروض وقت أو مهارة تنتظر المراجعة.');
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: const TextStyle(color: RuhamaaColors.primaryDark, fontWeight: FontWeight.w700),
                      ),
                      if (row['availability_note'] != null) ...[
                        const SizedBox(height: 4),
                        Text('التوفر: ${row['availability_note']}', style: const TextStyle(color: RuhamaaColors.textMuted)),
                      ],
...
class _OpenRequestsTabState extends State<_OpenRequestsTab> {
  ...
  Future<void> _openCandidates(Map<String, dynamic> request) async {
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر فتح المرشحين: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState != ConnectionState.done) return const Center(child: CircularProgressIndicator());
        if (snapshot.hasError) return Center(child: Text('تعذر تحميل طلبات الخدمات: ${snapshot.error}'));
        final rows = snapshot.data ?? const [];
        if (rows.isEmpty) return const _AdminEmpty('لا توجد طلبات خدمات مفتوحة الآن.');
        return RefreshIndicator(
          onRefresh: () async => setState(_reload),
          child: ListView.separated(
            padding: const EdgeInsets.all(16),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (row['preferred_time_note'] != null) ...[
                        const SizedBox(height: 4),
                        Text('الوقت المناسب: ${row['preferred_time_note']}', style: const TextStyle(color: RuhamaaColors.textMuted)),
                      ],
                      const SizedBox(height: 14),
                      FilledButton.icon(
...
class _CandidateSheetState extends State<_CandidateSheet> {
  ...
  Future<void> _match(Map<String, dynamic> candidate) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم إنشاء مطابقة خدمة خاصة. يمكن الآن استكمال تنسيق الموعد.')),
      );
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر إنشاء المطابقة: $e')));
    }
  }
            ...
            children: [
              Center(
                child: Container(
                  width: 42,
                  height: 4,
                  decoration: BoxDecoration(color: RuhamaaColors.border, borderRadius: BorderRadius.circular(99)),
                ),
              ),
              const SizedBox(height: 16),
              Text('مرشحون لـ «${widget.request['title']}»', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Expanded(
                                  child: Text('${candidate['title']}', style: const TextStyle(fontWeight: FontWeight.w900)),
                                ),
                                Chip(label: Text('نقاط ملاءمة ${candidate['score']}')),
                              ],
                            ),
                            const SizedBox(height: 5),
                            Text('${candidate['service_type']} • ${candidate['available_hours']} ساعة', style: const TextStyle(color: RuhamaaColors.textMuted)),
                            const SizedBox(height: 10),
                            FilledButton(
                              onPressed: () => _match(candidate),
                              child: const Text('اعتماد هذه المطابقة'),
                            ),
                          ],
                        ),
                      ),
...
class _AdminEmpty extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        child: Padding(
          ...
          child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: RuhamaaColors.textMuted, height: 1.5)),
        ),
      );
}
