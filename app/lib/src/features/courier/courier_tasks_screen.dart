...
class _CourierTaskScreenState extends State<CourierTaskScreen>{
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:Text(widget.task.publicCode)),body:FutureBuilder<CourierTaskDetails>(future:_details,builder:(context,s){if(s.connectionState!=ConnectionState.done)return const Center(child:CircularProgressIndicator());if(s.hasError)return const Center(child:Text('تعذر تحميل تفاصيل المهمة. حاول مجددًا.'));final d=s.data!;final pickup=d.status=='assigned'||d.status=='heading_to_pickup'||d.status=='rescheduled';return ListView(padding:const EdgeInsets.all(20),children:[
    const Text('تظهر لك البيانات الضرورية لتنفيذ المهمة فقط.',style:TextStyle(fontWeight:FontWeight.w600)),const SizedBox(height:20),
