...
begin
  ...
  if v_need.status not in ('submitted','waiting') then
    raise exception 'need is not eligible for discovery while a match offer or fulfillment is active';
  end if;
...
as $$
begin
  if new.status in ('candidate_found','confirmed','matched','delivery_scheduled','fulfilled','cancelled','expired') then
