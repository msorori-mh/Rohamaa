...
(async () => {
  ...
  try {
    ...
    await page.keyboard.press('Escape'); assert.equal(await menu.getAttribute('aria-expanded'),'false'); assert(await menu.evaluate(e=>e===document.activeElement)); assert.equal(await page.locator('#main-nav').isVisible(),false);
